...
my $help = 0;
my %payload;
my $usage = "$0 [options] [ -p probe_name ] -f ipsfile

Options are:
	--file file       File of ips - alternative to range
...
unless ($file) {
	$ip_range_start = shift or die $usage;
	$ip_range_end = shift or die $usage;
}
...
foreach my $probe_name_to_use (@probes_to_use) {
	...
	$scanner->add_target_ips_from_range($ip_range_start, $ip_range_end) unless $file;
